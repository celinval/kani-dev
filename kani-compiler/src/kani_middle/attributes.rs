    ...
    let check_resolve = |attr: &Attribute, path: &TypePath| {
        ...
        match result {
            ...
            Err(err) => {
                tcx.dcx().span_err(
                    attr.span,
                    format!("failed to resolve `{}`: {err}", pretty_path(path)),
                );
            }
        }
    };
...
/// Return a string for path without spaces.
///
/// `quote!()` and `TokenString::to_string()` introduce unnecessary space around the "::" separator.
fn pretty_path(path: &TypePath) -> String {
    let with_space = path.to_token_stream().to_string();
    with_space.replace(" :: ", "::")
}
