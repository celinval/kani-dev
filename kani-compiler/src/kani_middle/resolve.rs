...
pub fn resolve_fn_path<'tcx>(
    ...
    match (&path.qself, &path.path.leading_colon) {
        (Some(qself), Some(_)) => {
            ...
            Err(ResolveError::UnsupportedPath { kind: "qualified bare function" })
        }
        (Some(qself), None) => {
            let ty = resolve_ty(tcx, current_module, &qself.ty)?;
            let def_id = resolve_path(tcx, current_module, &path.path)?;
            validate!(tcx, def_id, "function / method", DefKind::Fn | DefKind::AssocFn)?;
...
pub fn resolve_fn<'tcx>(
    ...
    if let FnResolution::Fn(def) = result {
        ...
        Err(ResolveError::UnsupportedPath { kind: "qualified" })
    }
}
...
pub fn resolve_ty<'tcx>(
    ...
    let path = match typ {
        ...
        Type::ImplTrait(_) => {
            return invalid("ImplTrait");
        }
        Type::Infer(_) => {
            return invalid("infer");
        }
        Type::TraitObject(_) => {
            return invalid("trait object");
        }
        _ => {
            unreachable!()
        }
    ...
    validate!(tcx, def_id, "type", DefKind::Struct | DefKind::Union | DefKind::Enum)?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ResolveError::UnsupportedPath { kind } => {
                write!(f, "Kani currently does not support {kind} path")
            }
        }
    }
}
