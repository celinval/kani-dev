...
impl KaniSession {
    ...
    pub fn instrument_model(
        ...
        let restrictions =
            project.get_crate_artifacts(&harness.crate_name, ArtifactType::VTableRestriction);
        if !restrictions.is_empty() {
            assert_eq!(restrictions.len(), 1);
            self.apply_vtable_restrictions(&output, restrictions.first().unwrap().as_ref())?;
        }
        ...
        if self.args.gen_c {
            ...
            let symtabs = project.get_crate_artifacts(&harness.crate_name, ArtifactType::SymTab);
